    ...
    >
      ...
      >
        ...
        <div className="text-center mb-14">
          ...
          <p className="text-gray-400 text-lg">FreshGuard Inc. — Series A Funding Round</p>
        </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center py-3 border-b border-white/10 last:border-0">
                  ...
                  <span className={`font-black text-lg ${item.highlight ? 'text-brand' : 'text-white'}`}>
                    {item.value}
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-4">
              ...
              <div className="mt-4 p-4 bg-brand/10 rounded-2xl border border-brand/20">
                <p className="text-gray-300 text-sm leading-relaxed">
                  🔓 Premium unlocks advanced recipe suggestions, multi-fridge support, and detailed analytics.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur rounded-3xl border border-white/10 p-7 mb-6">
          ...
          <div className="space-y-3">
            {expenses.map((expense) => (
              <div key={expense.label} className="flex justify-between items-center py-3 border-b border-white/10 last:border-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          {projections.map((proj) => (
            <div key={proj.year} className={`bg-white/5 backdrop-blur rounded-3xl border-2 ${proj.color} p-6 text-center`}>
              <div className="text-3xl mb-2">🚀</div>
              <p className="text-gray-400 text-xs uppercase tracking-widest mb-1">{proj.year} Revenue</p>
              <p className="text-white font-black text-2xl">{proj.revenue}</p>
              <p className="text-brand text-sm font-bold mt-1">Profit: {proj.profit}</p>
            </div>
          ))}
        </div>
      </div>
