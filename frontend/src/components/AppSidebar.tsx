...
import { X, Home, Wrench, DollarSign, Presentation, Vote, ShoppingCart } from 'lucide-react';
...
export default function AppSidebar({ isOpen, onClose, currentPage, onNavigate }: AppSidebarProps) {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      action: () => { onNavigate('store'); onClose(); },
    },
    {
      label: 'Sharks Vote',
      icon: <Vote size={20} />,
      action: () => { onNavigate('vote'); onClose(); },
    },
  ];
  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed top-0 left-0 h-full w-72 z-50 bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${
